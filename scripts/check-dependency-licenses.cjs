 ...
 *   restriction forward. AGPL-3.0 forbids conveying the work under added
 *   restrictions, so the two cannot both be satisfied. It arrives via
 *   `@wagmi/connectors`, which declares it as a hard dependency. These are banned by
 *   exact name, NOT by scope: most of `@metamask/*` is MIT or ISC and is legitimately
 *   needed.
 *
