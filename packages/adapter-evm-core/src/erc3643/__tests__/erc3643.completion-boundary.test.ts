/**
 * SF-1 / F4 — ERC-3643 write-completion boundary lock.
 *
 * The shared `runCapabilityWrite` skeleton returns a `WriteExecutionResult` carrying a
 * `completion` discriminant. ERC-3643 deliberately does NOT re-export that signal: its
 * capability contract is the plain `OperationResult`. These tests lock BOTH halves of that
 * decision so neither can silently drift:
 *
 *  1. `completion` never appears on an ERC-3643 write result (any mode) — the property is
...
const RELAYER_TX_ID = 'relayer-erc3643-sub-1';
...
function makeCapability(signImpl: ReturnType<typeof vi.fn>): ERC3643Capability {
...
describe('ERC-3643 write-completion boundary (SF-1 / F4)', () => {
  ...
  it('omits `completion` on the confirmed path — property absent, not undefined', async () => {
    ...
    const result = await capability.mint({ to: TO, amount: '1000' }, EOA_CONFIG);

    expect(result).toEqual({ id: TX_HASH });
    expect(
      Object.prototype.hasOwnProperty.call(result, 'completion'),
      'F4 violated: shared executor `completion` leaked onto the ERC-3643 result'
    ).toBe(false);
  ...
  it('omits `completion` on the submit-only relayer path too', async () => {
    ...
    const result = await capability.mint(
      { to: TO, amount: '1000' },
    ...
    expect(
      Object.prototype.hasOwnProperty.call(result, 'completion'),
      'F4 violated: submit-only `completion` leaked onto the ERC-3643 result'
    ).toBe(false);
    expect(Object.keys(result)).toEqual(['id']);
  });
