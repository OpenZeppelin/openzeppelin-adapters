...
| [`reverse-miss-fallback/`](./reverse-miss-fallback) | **002 SF-1** mocked Sepolia miss-fallback: bound empty → L1 primary, chain-agnostic `scopedToNetworkId` gate, `isEnsProvenance` enrichment note. |
...
> (SF-5), and mainnet-L1 reverse miss-fallback (002 SF-1).
