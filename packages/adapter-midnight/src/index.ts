...
export const ecosystemDefinition: EcosystemExport = {
  ...
  createRuntime: (profile: ProfileName, config: NetworkConfig, options) =>
    createRuntime(profile, asMidnightNetworkConfig(config), options),
  adapterConfig: midnightAdapterConfig,
  getExportBootstrapFiles: (context: AdapterExportContext) =>
    getMidnightExportBootstrapFiles(context),
};
