...
- [ ] T023 [P] [US3] Factory-creation test for `createERC3643`, including an idempotent-`dispose()` assertion (FR-016), in `openzeppelin-adapters/packages/adapter-evm-core/src/erc3643/__tests__/erc3643.factory.test.ts`
- [ ] T024 [P] [US3] Mocked-RPC read tests (balance/frozen/jurisdiction decode; `simulateTransfer` allowed + blocked shapes) in `openzeppelin-adapters/packages/adapter-evm-core/src/erc3643/__tests__/erc3643.reads.test.ts`
- [ ] T025 [P] [US3] Mocked-execution write tests (`mint`/`burn`/`transfer`/`freeze`/`unfreeze` calldata), revert→typed-error mapping (`RecipientNotVerified`/`ComplianceModuleRejected`/`HolderFrozen`/`InsufficientBalance`), and a malformed-amount → `InvalidAmount` negative test (FR-003a), in `openzeppelin-adapters/packages/adapter-evm-core/src/erc3643/__tests__/erc3643.writes.test.ts`
...
- [ ] T031 [P] [US5] Node import-graph / no-React-Wagmi assertion test for the three built sub-paths in `openzeppelin-adapters/packages/adapter-evm/test/ri-capabilities-subpath-isolation.test.ts` (FR-015, SC-003)
- [ ] T032 [P] [US5] Node server-side integration test (no bundler/DOM) that imports each sub-path, constructs the capability, and executes one representative read + one strategy-driven write against a mocked RPC + mocked injected `signAndBroadcast`, in `openzeppelin-adapters/packages/adapter-evm/test/ri-capabilities-subpath-runtime.test.ts` (SC-002)

### Implementation for User Story 5

- [ ] T033 [P] [US5] Add `erc3643`/`erc4626`/`irs` entries to `openzeppelin-adapters/packages/adapter-evm-core/tsdown.config.ts`
- [ ] T034 [P] [US5] Create thin re-export modules `openzeppelin-adapters/packages/adapter-evm/src/capabilities/erc3643.ts`, `erc4626.ts`, `irs.ts` (re-export factories from `adapter-evm-core`)
- [ ] T035 [US5] Add `erc3643`/`erc4626`/`irs` entries to `openzeppelin-adapters/packages/adapter-evm/tsdown.config.ts`
- [ ] T036 [US5] Add `./erc3643`/`./erc4626`/`./irs` `exports` entries (types + import + require shape) to `openzeppelin-adapters/packages/adapter-evm/package.json`
- [ ] T037 [US5] Re-export capabilities and register `erc3643`/`erc4626`/`irs` ONLY in `capabilityFactories` + `ecosystemDefinition.capabilities` in `openzeppelin-adapters/packages/adapter-evm/src/index.ts` (direct `CapabilityFactoryMap` / sub-path consumption; do NOT add them to any pre-composed profile — FR-021, Assumptions "Profiles unchanged") (FR-014)
- [ ] T038 [US5] Run the repo export/vite-config validation (`pnpm validate:vite-configs` + equivalent) and confirm it passes with the new entries (SC-007)
