...
- [X] T045 [P] [US6] Submit-then-poll behavioral test (custom injected `signAndBroadcast` callback) wired to an ERC-3643 write in `openzeppelin-adapters/packages/adapter-evm-core/src/erc3643/__tests__/erc3643.execution-strategy.test.ts` (SC-006, FR-018). The injected callback submits (intermediate id + `pendingRelayer` status) then polls to a confirmed hash; asserts the capability surfaces the *confirmed* hash as the operation id and that a poll-time failure rejects as a typed error.
- [X] T046 [P] [US6] Strategy-agnostic test composing a capability write with `EoaExecutionStrategy` and the existing `RelayerExecutionStrategy` behind the injected callback in `openzeppelin-adapters/packages/adapter-evm-core/src/erc3643/__tests__/erc3643.strategies.test.ts`. A thin `strategyToSignAndBroadcast(strategy, wallet)` adapter bridges any `AdapterExecutionStrategy` into the injected-callback shape; the real EOA strategy runs against a stub wallet, the Relayer strategy's `execute` is spied — proving the same capability/write composes with either with zero capability-side change (assembled calldata, config, and session API key flow through unchanged).

### Implementation for User Story 6

- [X] T047 [US6] Add a dependency-graph assertion that the adapter package contains no Relayer-plugin-runtime dependency (`PluginContext`, `api.sendTransaction`) in `openzeppelin-adapters/packages/adapter-evm/test/no-plugin-runtime-dep.test.ts` (FR-011, SC-005). Build-free static walk of the three RI capability sub-path graphs + adapter-evm re-exports asserts no plugin-runtime surface (`PluginContext`/`api.sendTransaction`), no relayer-plugin-runtime package import, and no such dependency in `adapter-evm/package.json` (the client `@openzeppelin/relayer-sdk` is allowed — it is not a plugin runtime).
- [X] T048 [US6] Record the FR-018 verification outcome (confirmed, per research.md R6) in the capability JSDoc / a short note in `openzeppelin-adapters/packages/adapter-evm-core/src/capabilities/` (no new primitive added). Documented on the shared `SignAndBroadcast` type JSDoc in `capabilities/helpers.ts` (the load-bearing injected-callback contract): same shape as `ExecutionCapability.signAndBroadcast` + optional `waitForTransactionConfirmation` already accommodates submit-then-poll; existing EOA/Relayer strategies compose behind it; no new `@openzeppelin/ui-types` primitive added.
