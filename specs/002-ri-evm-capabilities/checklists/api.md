...
- [x] CHK002 Are the typed error classes enumerated by name, stable `code`, and required structured-detail fields, rather than described only as "typed error classes"? [Completeness, Spec §FR-012] — RESOLVED: new FR-012a enumerates the base class + 9 concrete classes (incl. `InvalidAmount`) with codes and detail fields.
