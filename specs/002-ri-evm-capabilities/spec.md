...
**Independent Test**: Can be fully tested by implementing a trivial injected `signAndBroadcast` callback that returns a tx hash immediately, paired with a `waitForTransactionConfirmation` that resolves after a simulated poll, wiring it into one capability write method, and asserting the capability returns a confirmed result through the async two-step flow.

**Acceptance Scenarios**:

1. **Given** a capability write method and a custom injected `signAndBroadcast` callback that submits then polls, **When** the write executes, **Then** the capability obtains the tx hash from `signAndBroadcast` and the confirmation from `waitForTransactionConfirmation` without shape mismatch.
...
- **Malformed amount input**: a non-numeric, fractional, negative, or sign/scientific-notation amount `string` is rejected with a typed error at the factory boundary before any RPC or submission (FR-003a), never coerced or sent on-chain.
...
- **FR-002**: Each new capability interface MUST extend `RuntimeCapability` and be classified Tier 3, mirroring `AccessControlCapability` (which is a direct promotion of a chain-agnostic service interface). Per the Plugin tech doc §2e, each capability spans Tier-2-style reads (RPC-backed) and Tier-3 execution-backed writes within a single interface, exactly as `AccessControlCapability` does. Each is a single capability classified at its highest tier (Tier 3); the read/write split is internal to one interface and MUST NOT be modeled as two separate capabilities or two `CapabilityFactoryMap` entries.
...
- **FR-003a**: All token/share amount values in the capability interfaces (method parameters and return values — e.g. mint/burn/transfer amounts, vault assets/shares, balances, conversion results, total assets) MUST be represented as base-unit decimal `string`. The interfaces MUST NOT expose `bigint`; the viem-based factory converts `string ↔ bigint` internally at the chain boundary. A base-unit decimal `string` is a base-10 integer string of the value in the contract's smallest unit: non-negative, no sign, no decimal point, no scientific notation, and no separators (e.g. `"1000000000000000000"`). The factory MUST reject a malformed, fractional, or negative amount string with the `InvalidAmount` typed error (FR-012a) before any RPC or submission, rather than silently coercing it.
...
- **FR-006**: `ERC3643Capability` MUST declare methods for: reads `balanceOf` (→ `Amount`), `isVerified` (→ `boolean`), `isFrozen` (→ `boolean`), `getJurisdiction` (→ jurisdiction-code `string`, or `undefined` when unknown), and `simulateTransfer` (→ `{ allowed, modulesEvaluated, blockingModule? }`); writes `mint`, `burn`, `transfer`, `freeze`, `unfreeze` (→ structured operation result). All read return shapes MUST be chain-agnostic types (FR-003) and all amounts base-unit `string` (FR-003a).
- **FR-007**: `ERC4626Capability` MUST declare methods for: reads `convertToAssets` (→ `Amount`), `convertToShares` (→ `Amount`), `totalAssets` (→ `Amount`); writes `deposit`, `withdraw` (→ structured operation result). Any shares-issued / assets-returned value included in a write result MUST also be a base-unit `string` (FR-003a), present when the receipt exposes it and omitted otherwise.
- **FR-008**: `IRSCapability` MUST declare methods for the full set of on-chain identity primitives: deploy a holder's ONCHAINID (→ operation result including the deployed `onchainId` address), register the trusted issuer (idempotent — see FR-008c), attach a claim, register the identity in the IRS (`registerIdentity`, accepting the holder's numeric jurisdiction / `country` code), look up a holder's ONCHAINID (`getOnchainId` → the lookup result of FR-008b), `isVerified` (→ `boolean`), and jurisdiction query (`getJurisdiction` → jurisdiction-code `string`, or `undefined` when unknown). The numeric `country` supplied to `registerIdentity` is the same value that `getJurisdiction` reads back as a jurisdiction code. The capability exposes these as discrete primitives; the multi-step onboarding *orchestration* (ordering, when to run trusted-issuer setup, opening-supply mint) is the consumer's responsibility (out of scope here).
- **FR-008a**: The claim-attachment method MUST accept a pre-signed claim (`topic`, `scheme`, `data`, `signature`, plus optional issuer identity address) and MUST NOT hold, accept, or use the trusted-issuer signing key — issuer-key custody and claim signing remain the consumer's responsibility. The capability MAY expose a pure, key-free helper that constructs the canonical claim payload/digest for the consumer to sign; when present, it MUST accept `{ onchainId, topic, scheme, data }` and return `{ digest, topic, scheme, data }`, performing no RPC, no signing, and no key access.
- **FR-008b**: `getOnchainId` MUST return a documented lookup result of shape `{ found: boolean; onchainId?: string }` — `{ found: false }` when the holder has no identity contract (never a throw), and `{ found: true, onchainId }` otherwise — so callers can branch on first-time onboarding.
- **FR-008c**: Idempotency MUST be defined observably: re-running `registerTrustedIssuer` (when already registered) and re-running `attachClaim` (for an already-attached claim) MUST complete successfully without throwing. Re-running `registerIdentity` for an already-registered holder MUST surface the `IdentityAlreadyRegistered` typed error (a recognizable already-complete signal), not an opaque revert, so the orchestrator can treat it as a no-op during partial-failure recovery.
...
- **FR-010**: The capability implementations MUST perform all chain reads through the adapter's network/read primitives and all writes through the existing `ExecutionCapability` (selected via execution config), exactly as `AccessControlCapability` does. The concrete factory + wallet-decoupling shape this implies is specified in FR-010a.
...
- **FR-012a**: The error model MUST define an abstract base class (`RICapabilityError`, carrying an optional `contractAddress`) plus the following concrete classes, each with a stable `code` and structured details (final class/code names confirmable in implementation, but the set and shape are required):
  - `RecipientNotVerified` — `RECIPIENT_NOT_VERIFIED` — details: `holder`
  - `ComplianceModuleRejected` — `COMPLIANCE_MODULE_REJECTED` — details: `blockingModule`
  - `HolderFrozen` — `HOLDER_FROZEN` — details: `holder`
  - `InsufficientBalance` — `INSUFFICIENT_BALANCE` — details: `holder`, `requested`, `available?`
  - `InsufficientShares` — `INSUFFICIENT_SHARES` — details: `holder`, `requested`, `available?`
  - `IdentityAlreadyRegistered` — `ALREADY_ONBOARDED` — details: `holder`, `onchainId?`
  - `IdentityOperationFailed` — `IRS_OPERATION_FAILED` — details: `operation`, `cause?`
  - `InvalidAmount` — `INVALID_AMOUNT` — details: `value`, `reason` — thrown at the factory boundary when an amount `string` is malformed, fractional, negative, signed, or in scientific notation (FR-003a), before any RPC or submission.
  - `RICapabilityOperationFailed` — `OPERATION_FAILED` — details: `operation`, `cause?` — the fallback for an UNmapped on-chain revert, which MUST be surfaced with enough context to diagnose and MUST NOT be silently swallowed.
...
- **FR-018**: The feature MUST verify that the existing execution contract accommodates an asynchronous submit-then-poll write model — specifically `ExecutionCapability.signAndBroadcast` plus its optional `waitForTransactionConfirmation(txHash)`, the injected-callback shape `createAccessControl(config, { signAndBroadcast })` consumes, rather than the wallet-bound `AdapterExecutionStrategy.execute(...)`. The outcome MUST be recorded (confirmed, or a documented interface gap) — without adding Relayer coupling to the adapter. (Outcome: confirmed — see `research.md` R6.)
...
- **Execution contract (injected `signAndBroadcast`)**: Write methods reuse the existing execution contract exactly as `createAccessControl(config, { signAndBroadcast })` does — the factory accepts an injected `signAndBroadcast` callback `(transactionData, executionConfig, onStatusChange?, runtimeApiKey?) => Promise<{ txHash; result? }>`. The asynchronous submit-then-poll model is expressed by `ExecutionCapability.signAndBroadcast` plus its optional `waitForTransactionConfirmation(txHash)`; the wallet-bound `AdapterExecutionStrategy.execute(...)` is one concrete strategy behind that contract, not the seam these capabilities depend on. The strategy is supplied by the consumer (the plugin supplies its own, out of scope); the capability is strategy-agnostic and never depends on `WalletCapability`.
...
- **SC-004**: Every read and write method across the three capabilities (ERC-3643: 5 reads incl. `simulateTransfer` + 5 writes; ERC-4626: 3 reads + 2 writes; IRS: 4 writes — deploy ONCHAINID, trusted-issuer registration, claim attachment, `registerIdentity` — + 3 reads — `getOnchainId` lookup, `isVerified`, jurisdiction) has at least one passing mocked-behavior test — verified by the adapter test suite.
...
- **SC-006**: A capability write composed with a test submit-then-poll injected `signAndBroadcast` callback (the `ExecutionCapability` shape, per FR-018) returns a confirmed result through the async two-step flow, and the same write composed with the existing EOA and Relayer strategies behind that callback also succeeds — verified by behavioral tests, confirming the FR-018 design check.
...
- **SC-008**: The capability interfaces are stable enough for the RI plugin to type its `Capabilities` port against them and scaffold route handlers without further interface changes during Week 1 — verified objectively by the published type surface matching the method/return shapes documented in `contracts/erc3643-capability.md`, `contracts/erc4626-capability.md`, and `contracts/irs-capability.md`, and behaviorally by the plugin team consuming the published (or pre-release) types without requesting interface reshaping.
