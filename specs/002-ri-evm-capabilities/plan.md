...
- **`@openzeppelin/ui-types`** (openzeppelin-ui) — define the three capability interfaces (each extending `RuntimeCapability`), their chain-agnostic domain types, and typed error classes. (`CapabilityFactoryMap` registration is deferred — the `(config) => Capability` factory signature can't carry the per-deployment options these capabilities require; see issue #42.)
...
- **`@openzeppelin/adapter-evm`** — re-export the three capabilities as direct sub-path exports (`/erc3643`, `/erc4626`, `/irs`) with matching `package.json` exports + tsdown entries. (Runtime `capabilityFactories`/`ecosystemDefinition.capabilities` registration is deferred — these capabilities need per-deployment options the map's `(config) => Capability` signature can't supply; see issue #42 and `docs/ADAPTER_ARCHITECTURE.md`.)
...
|-----------|--------|-------|
| I. Capability-Compliant, Adapter-Led Architecture | PASS | Three new capabilities implement interfaces defined in `@openzeppelin/ui-types`; factories live in `adapter-evm-core`, re-exported by `adapter-evm` via sub-path exports. Mirrors the `AccessControlCapability` reference. Runtime `CapabilityFactoryMap`/`ecosystemDefinition` registration is deferred (per-deployment options incompatible with the current factory signature; see issue #42) — consumed directly via sub-paths for now. |
