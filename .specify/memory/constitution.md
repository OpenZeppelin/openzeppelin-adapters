...
Added Sections:
- I. Interface-Compliant, Adapter-Led Architecture
- II. Chain-Specific Encapsulation
- III. Type Safety & Code Quality
- IV. Consumer-First API Design
- V. Shared Core & Reuse-First Development
- VI. Testing & Test-Driven Development
- VII. Packaging, Build Integration & Release Management
- Additional Constraints
- Development Workflow and Review Process
- Governance
Templates:
- ✅ plan-template.md — Constitution Check placeholder is dynamic; no update needed
- ✅ spec-template.md — generic template; no principle-specific references
- ✅ tasks-template.md — generic template; no principle-specific references
...
### I. Interface-Compliant, Adapter-Led Architecture (NON-NEGOTIABLE)

- Every adapter package MUST implement the `ContractAdapter` interface from
  `@openzeppelin/ui-types` and be instantiated with a `NetworkConfig`.
- Adapters are published under the `@openzeppelin/adapter-*` namespace as
  independently versioned npm packages.
- The `@openzeppelin/adapter-evm` package is the reference implementation; all
  new adapters MUST mirror its structure, naming conventions, and export
  patterns. `@openzeppelin/adapter-stellar` mirrors EVM wherever applicable.
- Each adapter MUST expose a consistent public surface: contract loading/schema
  parsing, type mapping, input/output transforms, read support, transaction
  flows, wallet integration, and network configuration.
- Adapters MUST NOT add methods beyond those defined by `ContractAdapter` unless
  coordinated with `openzeppelin-ui` maintainers and validated by the
  `lint:adapters` check.
- `@openzeppelin/adapter-evm-core` centralizes reusable EVM logic (ABI loading,
  schema transformation, proxy handling, input/output conversion, transaction
  formatting, wallet infrastructure) shared by `adapter-evm` and
  `adapter-polkadot`.
- Rationale: Guarantees that consuming applications (UI Builder, OpenZeppelin UI,
  Role Manager, RWA Wizard) can swap or compose adapters without knowledge of
  chain internals, preserving ecosystem neutrality.
...
- Breaking changes to the `ContractAdapter` interface or any exported adapter API
  MUST:
  ...
  - Be coordinated with `openzeppelin-ui` maintainers when the interface itself
    changes.
...
- `pnpm lint:adapters` validates adapter implementations against the
  `ContractAdapter` interface; this check MUST pass before merge.
...
- Code review enforces:
  - Interface compliance (no extra methods beyond `ContractAdapter`).
...
- New adapter contributions MUST follow the Adapter Architecture Guide
  (`docs/ADAPTER_ARCHITECTURE.md`): network-aware constructor, standardized
  exports, ecosystem registration, and strict interface compliance.
...
- Breaking changes to adapter public APIs or the `ContractAdapter` interface
  MUST:
...
- CI enforces compliance; PRs violating constitutional rules MUST be corrected
  before merge.
- Version Policy:
  - MAJOR: Breaking changes to public APIs or `ContractAdapter` interface.
